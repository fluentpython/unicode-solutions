...
class UnicodeNameIndex:
    ...
    def query_iter(self, query):
        ...
        if query.startswith('?'):
            query = query[1:]
            return QueryResult(len(query), query)
        else:
            return self.find_chars(query)
...
def main(*args):
    ...
    query = ' '.join(args)
    n = 0
    result = index.query_iter(query)
    for n, char in enumerate(result.items, 1):
        print(index.describe_str(char))
